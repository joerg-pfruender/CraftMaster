...
class OnOffButton():
    ...
    def _changeState(self, state):
        ...
        if state:
            ...
            self.slideQuad.vertices = Shape2D.quad_vertices(self.x + self.width // 8, self.y - 5 * self.height // 8,
                                                            self.width // 4, 5 * self.height // 4)
        else:
            # change to Left(True)
            self.slideQuad.vertices = Shape2D.quad_vertices(self.x - 3 * self.width // 8, self.y - 5 * self.height // 8,
                                                            self.width // 4, 5 * self.height // 4)
        self.state = state
